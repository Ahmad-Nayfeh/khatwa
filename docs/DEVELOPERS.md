...
## الحساب والنسخة السحابية

- الحساب اختياري (أول شاشة في الترحيب ويمكن تخطيها، أو الإعدادات ← الحساب، أو تبويب المجموعات): بريد وكلمة مرور عبر Firebase Auth.
- عند تسجيل الدخول تُحفظ نسخة من ملف النسخ الاحتياطي (gzip + base64) في `users/{uid}/backup/latest`، ثم يومياً (`CloudBackupWorker`). القواعد تسمح بها لصاحبها فقط، حتى المشرف لا يقرؤها.
- بعد إعادة التثبيت: تسجيل الدخول في الترحيب يسترجعها فوراً؛ من الإعدادات يسأل التطبيق أولاً لأنها تستبدل بيانات الجوال.

## الإشراف

- لوحة الإشراف تظهر (الإعدادات ← الحساب، وتبويب المجموعات) لمن يملك وثيقة `admins/{uid}`. تُنشأ الوثيقة مرة واحدة من التطبيق بكتابة «كلمة مرور الإشراف».
- كلمة المرور يختارها المشرف من لوحة الإشراف: تُحفظ بصمتها فقط في `config/admin.keySha256`. قبل ذلك تُقبل بصمة المفتاح المؤقت `adminKeySha256()`، وبعدها لا.
...
- المشرف يقرأ ويعدّل (`isAdmin()`): `groups/**` (كل المجموعات حتى المخفية، الأعضاء، الأرقام، أيام الرسم)، `invites/**`، `admins/**`، `config/**`، والملفات الشخصية `users/{uid}` وعضوياتها. النسخة السحابية `users/{uid}/backup` ليست منها.
- الاختبارات (`firebase/tests` وCI) تستبدل البصمة ببصمة مفتاح اختبار معروف في نسخة مؤقتة من القواعد.
- إن ضاعت كلمة المرور: احذف وثيقة `config/admin` من لوحة Firebase، فيعود المفتاح المؤقت صالحاً (أو ضع بصمة مفتاح جديد في `adminKeySha256()` وانشر القواعد).
